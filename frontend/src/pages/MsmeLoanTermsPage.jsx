...
// Dedicated MSME loan-terms page, reached from the ESR page's "Apply for this
// Loan" button. The MSME states how much they need and for how long before
// the case goes to the Cred2Tech admin queue; the assigned DSA prepares the
// actual proposal against these stated terms (and the lender's real eligible
// amount/ROI/tenure).
export default function MsmeLoanTermsPage() {
  const { id: caseId } = useParams();
  const navigate = useNavigate();
  const location = useLocation();
  const lender = location.state?.lender || null;
  ...
  const maxAmount = lender?.eligible_amount || null;
  const maxTenure = lender?.tenure_months || null;
  const maxRate = lender?.roi || null;

  useEffect(() => {
    if (lender) {
      setAmount(lender.eligible_amount ? String(Math.round(lender.eligible_amount)) : '');
      setTenure(lender.tenure_months ? String(lender.tenure_months) : '');
      setRate(lender.roi ? String(lender.roi) : '');
    }
  }, [lender]);
    ...
    return (
      <div style={{ maxWidth: 640, margin: '0 auto' }}>
        <div className="card" style={{ padding: '60px 40px', textAlign: 'center' }}>
          ...
          <button className="btn btn-primary" onClick={() => navigate(`/cases/${caseId}/esr`)}>
  ...
  const handleSubmit = async () => {
    ...
    try {
      setSubmitting(true);
      await msmeApi.selectLender(lender.id);
    ...
    <div style={{ maxWidth: 640, margin: '0 auto' }}>
      ...
      <div className="card" style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 18 }}>
        <div>
          ...
          {maxAmount != null && (
            <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginTop: 4 }}>Eligible up to {formatDynamicCurrency(maxAmount)}</div>
          )}
        </div>
        <div>
          ...
          {maxTenure != null && (
            <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginTop: 4 }}>Max tenure {formatDynamicTenure(maxTenure)}</div>
          )}
        </div>
        <div>
          ...
          {maxRate != null && (
            <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginTop: 4 }}>Lender rate {maxRate}% p.a.</div>
          )}
        </div>
        ...
        {emi > 0 && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, padding: '14px', background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 8 }}>
            <div>
              ...
              <div style={{ fontSize: 16, fontWeight: 800, color: '#2B6CB0' }}>{formatDynamicCurrency(emi)}</div>
            </div>
            <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 24 }}>
        <button className="btn btn-ghost" type="button" onClick={() => navigate(`/cases/${caseId}/esr`)} disabled={submitting}>
