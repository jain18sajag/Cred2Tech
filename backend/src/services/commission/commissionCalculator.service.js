 ...
 */
function calculateCommission(disbursement, sanction, rule) {
    const disbursedAmount = new Decimal(disbursement.amount);
    ...
    // Formula: (disbursed_amount * baseRate) / 100
    const calculatedCommission = disbursedAmount.mul(baseRate).div(100);
