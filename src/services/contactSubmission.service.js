...
const contactSubmissionService = {
  ...
  create: async ({ fullName, businessName, mobileNumber, email, role, helpType, message }, ipAddress) => {
    ...
    if (!helpType || !helpType.trim()) throw new Error('Enquiry type is required.');
    ...
    return prisma.contactSubmission.create({
      data: {
        ...
        help_type: helpType.trim().slice(0, 50),
  ...
  getUnreadCount: async () => prisma.contactSubmission.count({ where: { is_read: false } }),
...
module.exports = { contactSubmissionService, isAdminRole, ADMIN_ROLES };
