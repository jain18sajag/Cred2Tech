...
function buildManualIncomeImpactLog(esr) {
    ...
    const rows = entries.map(entry => {
        ...
        const isBankCredit = isRent && (type.includes('bank') || docText.includes('bank') || docText.includes('credit'));
        ...
        return {
            ...
            impactsAutoBestMethod: salariedEligible > 0 || npmEligible > 0 || bankingEligible > 0 || gstEligible > 0,
            note
        };
    });
    ...
    return {
        ...
        formula: 'annualAmount / 12, then lender/scheme policy percentage is applied. Banking selected method remains unaffected by manual income.',
        entries: rows,
        totals,
        warnings: rows
            .filter(row => row.policyDecision.includes('EXCLUDED') || row.policyDecision.includes('MANUAL_REVIEW'))
...
function buildPropertyCollateralLog(esr, lenderResults = []) {
    ...
    return {
        ...
        warnings: [
            ...
            !(esr.ownership || esr.ownership_status || esr.property_ownership) ? 'Ownership is captured for audit, but current ICICI code does not reduce eligibility by ownership.' : null,
